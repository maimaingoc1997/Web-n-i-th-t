...
<?php 
    ...
    $sql_khachhang = mysqli_query($conn,"INSERT INTO tbl_khachhang(name,  email,  password) VALUES ('$name','$email',  '$password')");
    $sql_khachhang = mysqli_query($conn, "SELECT * FROM tbl_khachhang ORDER BY khachhang_id DESC LIMIT 1");
    $row_khachhang = mysqli_fetch_array($sql_khachhang);
    $_SESSION['dangnhap_home'] = $name;
    $_SESSION['khachhang_id'] = $row_khachhang['khachhang_id'];
    echo '<script language="javascript">alert("Đăng ký tài khoản thành công!");</script>
